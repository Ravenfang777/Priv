...
-- Exact-HP and speed controller used by the Wakka private-theme v4 test.
--
...
--
-- Install this file by itself. Remove every standalone Enemy Stats Manager,
-- Multi-Enemy Battle Themes, BGM Identifier Recorder, and older Wakka
-- stat/speed/theme script. They own the same hooks or would modify targets
-- twice. Fully restart KH1FM after changing installed scripts.
--
...
LUAGUI_AUTH = "OpenAI"
LUAGUI_DESC = "Exact HP and per-animation/overall speed for every named enemy"
...
-- BATTLE_THEME    Retained for compatibility, but disabled in this test.
--                 The companion private-theme loader handles Wakka's song.
--
-- Wakka contains the verified example. All other enemies use native values.

local ENEMY_SETTINGS = {
    ["Shadow"] = { MAX_HP = 1, ANIMATION_SPEED = {}, OVERALL_SPEED = 1.5, BATTLE_THEME = 110 },
    ...
    ["Riku - Wooden Sword"] = { MAX_HP = 500, ANIMATION_SPEED = {}, OVERALL_SPEED = 1.3, BATTLE_THEME = 113 },
    ["Tidus"] = { MAX_HP = 300, ANIMATION_SPEED = {}, OVERALL_SPEED = 1.3, BATTLE_THEME = 112 },
    ["Selphie"] = { MAX_HP = 200, ANIMATION_SPEED = {}, OVERALL_SPEED = 1.2, BATTLE_THEME = 111 },
    ["Wakka"] = {
        MAX_HP = 300,
...
-- The normal numbered-theme engine is intentionally not initialized in this
-- test. It would require replacing music110 globally and would therefore
-- change the menu. The companion loader owns the one available BGM frame hook
-- and loads Wakka's private SCD only after Wakka is confirmed.
local musicModule = {
    init = function() end,
    frame = function() end,
}
...
function _OnInit()
    ...
    -- Install the fixed enemy-stat cave first. The music cave search explicitly
    -- reserves that range, matching the previously verified two-script setup.
    statsModule.init()
    musicModule.init()
end

function _OnFrame()
    -- Music examines and remembers narrow lock-on exclusions before the stats
    -- module can apply fallback HP. The stats module independently enforces the
    -- same shared exclusion, so either order remains fail-closed.
    musicModule.frame()
    statsModule.frame()
end
