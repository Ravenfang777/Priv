...
-- File: ZZZZ_KHFM_Wakka_PrivateTheme_Loader_v4_1.lua
-- Enemy-specific private battle-theme loader / Wakka test v4.1
--
-- Verified target:
...
-- positively detected, a one-shot main-thread dispatcher loads the private
-- SCD beside this Lua through the game's Panacea-aware file manager, registers
-- that live buffer as a primary BGM resource, stops slot 0, and starts it.
--
...
LUAGUI_NAME = "KH1FM Wakka Private Theme Loader v4.1"
LUAGUI_AUTH = "OpenAI"
...
local SETTINGS = {
    ...
    REPORT_FILENAME = "KHFM_Wakka_Private_Theme_v4_1_Report.txt",
...
-- Native callers take this file-manager lock, call the Panacea-detoured
-- allocation loader, and then release the lock. The returned buffer and byte
-- length are passed to the verified BGM-resource registration routine before
-- playback.
...
local LOAD_FILE_WITH_MALLOC_RVA = 0x000D57E0
local PANACEA_DETOUR_SIGNATURE = {
...
local DATA_LOAD_PATH_OFFSET = 0x000
local DATA_LOAD_PATH_CAPACITY = 0x088
...
local DATA_MAGIC = "BGMWAK4\0"
local DATA_SIZE = 0x160

-- 274-byte wrapper assembled from
-- analysis_v8/wakka_theme_v2_4_dispatch.S. It consumes a Lua-published
-- one-shot request on the main game thread, loads music110 now through
-- Panacea, registers the returned live buffer, stops slot 0, plays slot 0,
-- records a definitive result code, and tail-jumps to LuaBackend's frame hook.
local FRAME_CODE_HEX =
    "514883ec40833d00000000000f84f5000000c6050000000000c605000000"
    .. "0001488b0d000000004885c90f84be000000e800000000c7050000000000"
    .. "000000488b0d00000000488d15000000004c8d05000000004531c948c744"
    .. "24200000000048c744242800000000e80000000048890500000000488b0d"
    .. "00000000e8000000004c8b05000000004d85c0746a488d0d0000000031d2"
    .. "448b0d00000000e80000000089050000000085c0745431c9e80000000031"
    .. "c94531c0448b0d00000000f30f100d00000000f30f1015000000004c8944"
    .. "24204c89442428e800000000f0ff0500000000c6050000000005eb19c605"
    .. "0000000002eb10c6050000000003eb07c60500000000044883c44059ff25"
    .. "00000000"
local FRAME_CODE_SIZE = 0x112
local FRAME_CODE_PREFIX = {
    0x51, 0x48, 0x83, 0xEC, 0x40, 0x83, 0x3D,
}
local FRAME_RELOCATIONS = {
    { field = 0x007, next = 0x00C, data = DATA_DISPATCH_FLAG_OFFSET },
    { field = 0x014, next = 0x019, data = DATA_DISPATCH_FLAG_OFFSET },
    { field = 0x01B, next = 0x020, data = DATA_DISPATCH_STATUS_OFFSET },
    { field = 0x023, next = 0x027, absolute = FILE_MANAGER_POINTER_RVA },
    { field = 0x031, next = 0x035, absolute = FILE_MANAGER_LOCK_RVA },
    { field = 0x037, next = 0x03F, data = DATA_LOAD_SIZE_OFFSET },
    { field = 0x042, next = 0x046, absolute = FILE_MANAGER_POINTER_RVA },
    { field = 0x049, next = 0x04D, data = DATA_LOAD_PATH_OFFSET },
    { field = 0x050, next = 0x054, data = DATA_LOAD_SIZE_OFFSET },
    { field = 0x06A, next = 0x06E, absolute = LOAD_FILE_WITH_MALLOC_RVA },
    { field = 0x071, next = 0x075, data = DATA_LOAD_BUFFER_OFFSET },
    { field = 0x078, next = 0x07C, absolute = FILE_MANAGER_POINTER_RVA },
    { field = 0x07D, next = 0x081, absolute = FILE_MANAGER_UNLOCK_RVA },
    { field = 0x084, next = 0x088, data = DATA_LOAD_BUFFER_OFFSET },
    { field = 0x090, next = 0x094, data = DATA_TARGET_NAME_OFFSET },
    { field = 0x099, next = 0x09D, data = DATA_LOAD_SIZE_OFFSET },
    { field = 0x09E, next = 0x0A2, absolute = REGISTER_BGM_RESOURCE_RVA },
    { field = 0x0A4, next = 0x0A8, data = DATA_TARGET_RESOURCE_OFFSET },
    { field = 0x0AF, next = 0x0B3, absolute = BGM_STOP_FUNCTION_RVA },
    { field = 0x0BB, next = 0x0BF, data = DATA_DISPATCH_TIME_OFFSET },
    { field = 0x0C3, next = 0x0C7,
        data = DATA_DISPATCH_VOLUME_BITS_OFFSET },
    { field = 0x0CB, next = 0x0CF,
        data = DATA_DISPATCH_FADE_BITS_OFFSET },
    { field = 0x0DA, next = 0x0DE, absolute = BGM_FUNCTION_RVA },
    { field = 0x0E1, next = 0x0E5, data = DATA_DISPATCH_COUNTER_OFFSET },
    { field = 0x0E7, next = 0x0EC, data = DATA_DISPATCH_STATUS_OFFSET },
    { field = 0x0F0, next = 0x0F5, data = DATA_DISPATCH_STATUS_OFFSET },
    { field = 0x0F9, next = 0x0FE, data = DATA_DISPATCH_STATUS_OFFSET },
    { field = 0x102, next = 0x107, data = DATA_DISPATCH_STATUS_OFFSET },
    { field = 0x10E, next = 0x112,
...
local function console(message)
    ConsolePrint("[WakkaThemeV2.4] " .. message)
end
...
local function buildReport()
    local lines = {
        "KH1FM Wakka Private Theme Test v4.1 report",
        ...
        "Playback: the private SCD is loaded and registered only when Wakka is confirmed.",
...
local function buildHookData()
    ...
    putString(
        data,
        DATA_LOAD_PATH_OFFSET,
        DATA_LOAD_PATH_CAPACITY,
...
local function installHooks()
    ...
    if frameCodeRva == nil then
        return false,
            "no safe executable cave was available for the packed "
                .. "274-byte load/register/play dispatcher; remove older "
                .. "Wakka theme scripts and BGM Recorder v8, then restart"
    end
    codeRva = frameCodeRva
    occupied[#occupied + 1] = {
        first = frameCodeRva,
        last = frameCodeRva + FRAME_CODE_SIZE,
    }
    ...
    return true, string.format(
        "installed Wakka on-demand BGM load/register/play dispatch "
            .. "frame_code=0x%X size=%u data=0x%X",
        frameCodeRva,
        FRAME_CODE_SIZE,
        dataRva
    )
...
function SETTINGS._queueAlreadyPlayingSwitch()
    ...
    writeIntChecked(dataRva + DATA_DISPATCH_STATUS_OFFSET, 0)
    local ok, reason = writeIntChecked(
    ...
    addTimeline(string.format(
        "ACTIVE SWITCH QUEUED tick=%u seconds=%.3f id=%d "
...
function SETTINGS._processDispatchCounter()
    ...
    lastLoadBuffer =
        safeReadLong(dataRva + DATA_LOAD_BUFFER_OFFSET, true) or 0
    ...
    if status ~= lastDispatchStatus then
        ...
        if status == 1 then
            addTimeline(string.format(
                "ACTIVE SWITCH DISPATCH ENTERED tick=%u seconds=%.3f",
                tick,
                tick / 60
            ), false)
        elseif status >= 2 and status <= 4 then
            local reasons = {
                [2] = "file-manager singleton unavailable",
                [3] = "private Wakka SCD load returned no buffer",
                [4] = "live BGM resource registration failed",
            }
            activeSwitchQueued = false
            addTimeline(string.format(
        ...
        elseif status == 5 then
            addTimeline(string.format(
                "REPLACEMENT LOAD READY tick=%u seconds=%.3f "
                    .. "name=%s size=%u buffer=0x%X resource=0x%08X",
                tick,
                tick / 60,
                SETTINGS.REPLACEMENT_BGM,
                lastLoadSize,
...
function SETTINGS._preparePrivateScdPath()
    ...
    if string.len(SETTINGS.PRIVATE_SCD_PATH)
        >= DATA_LOAD_PATH_CAPACITY
    then
        return false, "private SCD path is too long: "
            .. SETTINGS.PRIVATE_SCD_PATH
    end
...
function _OnInit()
    ...
    if not arraysEqual(
        safeReadArray(
            LOAD_FILE_WITH_MALLOC_RVA,
            #PANACEA_DETOUR_SIGNATURE
        ),
        PANACEA_DETOUR_SIGNATURE
    ) then
        addStatus(
            "DISABLED: Panacea LoadFileWithMalloc detour is unavailable.",
            true
        )
        saveReport()
        return
    ...
    addStatus(
        "READY: private Wakka SCD path verified; the original game "
            .. "music110 remains untouched for the menu.",
        true
    )
    addStatus(
        "Fight Wakka once. F2 should show WAKKA PRESENT, ROUTE ARMED, "
            .. "ACTIVE SWITCH QUEUED, REPLACEMENT LOAD READY, then "
            .. "ACTIVE SWITCH EXECUTED. Use a full game restart "
